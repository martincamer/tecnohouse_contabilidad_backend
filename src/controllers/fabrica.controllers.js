...
export const getTipos = async (req, res, next) => {
  //obtener perfiles
  const result = await pool.query("SELECT * FROM fabrica");
  return res.json(result.rows);
};

export const getTipo = async (req, res) => {
  const result = await pool.query("SELECT * FROM fabrica WHERE id = $1", [
    req.params.id,
  ]);
...
export const createTipo = async (req, res, next) => {
  ...
  try {
    const result = await pool.query(
      "INSERT INTO fabrica (tipo) VALUES ($1) RETURNING *",
      [tipo]
    );
...
export const actualizarTipo = async (req, res) => {
  ...
  const result = await pool.query(
    "UPDATE fabrica SET tipo = $1 WHERE id = $2",
    [tipo, id]
  );
...
export const eliminarTipo = async (req, res) => {
  const result = await pool.query("DELETE FROM fabrica WHERE id = $1", [
    req.params.id,
  ]);
